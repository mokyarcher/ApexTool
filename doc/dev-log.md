...
点击武器卡片后的详情面板改版，参考神话级页面 `PagedRow` 的设计：

- 顶部红色发光线条 + 渐变 Header（武器图标 + 类别 + 名称 + 皮肤数量徽章）
- 中间皮肤卡片横排展示（每页 4 张，16:10 横幅卡片）
- 左右翻页按钮（红色边框，长条形）+ 底部红色页码指示器
- 翻页动画：`animate-page-slide`（带方向感知）
- 关闭方式统一：右上角 ESC 按钮 + 真实键盘 ESC 键支持
- 键盘左右方向键支持翻页
- 皮肤数据先用占位图模拟（每把武器 15 款，4 页可翻），便于预览效果
...
## 2025-05-15 百科筛选 - 去除全部分类

### 传奇角色分类筛选

- 传奇角色页移除"全部"筛选按钮，只保留职业分类按钮
- 默认显示"突击手"分类，与武器页按分类展示逻辑保持一致

### 涉及文件

| 文件 | 改动 |
|------|------|
| `client/src/pages/Encyclopedia.jsx` | 传奇角色 roleFilter 默认值改为突击手，移除 roles 中的 all |

---

## 2026-05-15 商店收集活动 - 超频赛季收集

### 收集活动图源更新

- 将离线调试图源同步到 `client/public/shop/milestone/`
- 活动名称从"余震里程碑收集"更新为"超频赛季收集"
- 详情弹窗标题同步更新为"超频赛季收集"
...
| 文件 | 改动 |
|------|------|
| `server/data/milestone.json` | 更新活动中文名、英文名和详情标题 |
| `client/public/shop/milestone/` | 更新 banner、icon、featured、rewards 图源 |
...
## 2026-05-15 武器皮肤 3D 查看器横向平放修正

### 横向展示修正

- 移除 3D 查看器默认 `auto-rotate`，避免进入时模型继续转偏
- 将 `orientation` 调整为 `90deg 0deg 90deg`，让 R-99 枪体默认横向平放
- 将 `camera-orbit` 调整为 `0deg 75deg auto`

### 涉及文件

| 文件 | 改动 |
|------|------|
| `client/src/pages/WeaponSkins.jsx` | 修正 model-viewer 默认姿态参数 |
