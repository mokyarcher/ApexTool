...
try {
  ...
  $remoteCommand = "cd $remoteProjectPath && node server/scripts/import-ranked-leaderboard.js $remoteHtmlPath && npm --prefix client run build && pm2 restart apex-server --update-env"
