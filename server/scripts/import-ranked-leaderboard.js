...
const players = rows.map((row, index) => {
  ...
  const uid = firstMatch(playerCell, /profile\/uid\/[^/]+\/([0-9]+)/i) || firstMatch(playerCell, /load(?:Sum|Perf)\(&#39;([0-9]+)&#39;\)/i);
  const name = cleanText(firstMatch(playerCell, /<a[^>]*profile\/uid[^>]*>([\s\S]*?)<\/a>/i));
